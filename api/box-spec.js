// GENERATED โดย scripts/build-api.mjs — อย่าแก้ไฟล์นี้ตรง ๆ (แก้ที่ server/handler.ts แล้วรัน npm run build:api)
...
var rect = (xa, ya, xb, yb) => [
  P(xa, ya),
  P(xb, ya),
  P(xb, yb),
  P(xa, yb)
];
var fmt = (v) => String(Math.round(v * 10) / 10);
function obroundPts(cx, cy, length, thick, vertical = false) {
  const r = thick / 2;
...
function roundedRectPts(x0, y0, x1, y1, r) {
  return [
    P(x0 + r, y0),
...
function obroundPath(cx, cy, length, thick, vertical = false) {
  const r = thick / 2;
  const half = Math.max(0, length / 2 - r);
  if (!vertical) {
    return `M ${cx - half} ${cy - r} L ${cx + half} ${cy - r} A ${r} ${r} 0 0 1 ${cx + half} ${cy + r} L ${cx - half} ${cy + r} A ${r} ${r} 0 0 1 ${cx - half} ${cy - r} Z`;
  }
  return `M ${cx - r} ${cy - half} A ${r} ${r} 0 0 1 ${cx + r} ${cy - half} L ${cx + r} ${cy + half} A ${r} ${r} 0 0 1 ${cx - r} ${cy + half} Z`;
}

// src/core/templates/tuckEnd.ts
function generateTuckEndBox(box, mat) {
  const { W, D, H } = box;
  ...
  const panels = [
    ...
    },
    {
      ...
      holes: hasHandle ? [obroundPts(holeCx, holeCy, holeLen, holeThick)] : void 0,
      hingeA: P(x4, top),
      hingeB: P(x5, top),
      foldAngle: 90,
      stage: 2
    ...
    },
    {
      id: "tuck-bot",
      parentId: "back",
      outline: rect2(x4 + tuckIn, bot, x5 - tuckIn, tiBot),
      hingeA: P(x4, bot),
      hingeB: P(x5, bot),
      foldAngle: -90,
      stage: 2
    },
    {
      id: "tongue-bot",
      parentId: "tuck-bot",
      outline: tongueBotOutline,
      hingeA: P(x4 + tuckIn, tiBot),
      hingeB: P(x5 - tuckIn, tiBot),
      foldAngle: -90,
      stage: 3,
      zOffset: 0.15
    }
  ...
  const tuckCut = (y, dir) => {
    ...
    return `M ${x4} ${y} L ${x4 + tuckIn} ${y} L ${x4 + tuckIn} ${yi} L ${x4 + tuckIn} ${yt + dir * r} Q ${x4 + tuckIn} ${yt} ${x4 + tuckIn + r} ${yt} L ${x5 - tuckIn - r} ${yt} Q ${x5 - tuckIn} ${yt} ${x5 - tuckIn} ${yt + dir * r} L ${x5 - tuckIn} ${yi} L ${x5 - tuckIn} ${y} L ${x5} ${y}`;
  };
  const segments = [
...
// src/core/templates/mailer.ts
function generateMailerBox(box, mat) {
  const { W, D, H } = box;
  const t = mat.thickness;
  const Wp = W + 2 * t;
  const Dp = D + 2 * t;
  ...
  const panels = [
    ...
    },
    {
      id: "side-left",
      parentId: "base",
      outline: rect(cx0 - Hp, y3, cx0, y4),
      holes: hasHandle ? [obroundPts(holeLx, holeCy, holeLen, holeThick, true)] : void 0,
      hingeA: P(cx0, y3),
      hingeB: P(cx0, y4),
      foldAngle: -90,
      stage: 0
    },
    {
      id: "side-right",
      parentId: "base",
      outline: rect(cx1, y3, cx1 + Hp, y4),
      holes: hasHandle ? [obroundPts(holeRx, holeCy, holeLen, holeThick, true)] : void 0,
      hingeA: P(cx1, y3),
      hingeB: P(cx1, y4),
      foldAngle: 90,
      stage: 0
    ...
    },
    {
      ...
      parentId: "lid",
      outline: lipOutline,
      hingeA: P(cx0 + tuckIn, y1),
      hingeB: P(cx1 - tuckIn, y1),
      foldAngle: 90,
      stage: 3,
      zOffset: layer
    },
  ...
  const cut = (d) => ({ kind: "cut", d });
  const crease = (d) => ({ kind: "crease", d });
  const dustCut = (xa, xb, y, dir) => `M ${xa} ${y} L ${xa + dustIn} ${y} L ${xa + dustIn + dustSlant} ${y + dir * dustW} L ${xb - dustIn - dustSlant} ${y + dir * dustW} L ${xb - dustIn} ${y} L ${xb} ${y}`;
  const segments = [
    cut(
      `M ${cx0 + tuckIn} ${y1} L ${cx0 + tuckIn} ${r} Q ${cx0 + tuckIn} 0 ${cx0 + tuckIn + r} 0 L ${cx1 - tuckIn - r} 0 Q ${cx1 - tuckIn} 0 ${cx1 - tuckIn} ${r} L ${cx1 - tuckIn} ${y1}`
    ),
    cut(`M ${cx0} ${y1} L ${cx0 + tuckIn} ${y1}`),
    cut(`M ${cx1 - tuckIn} ${y1} L ${cx1} ${y1}`),
    cut(
      `M ${cx0} ${y1} L ${cx0} ${y1 + lipIns} L ${cx0 - lipS} ${y1 + lipIns + lipSlant} L ${cx0 - lipS} ${y2 - lipIns - lipSlant} L ${cx0} ${y2 - lipIns} L ${cx0} ${y2}`
    ),
    cut(
      `M ${cx1} ${y1} L ${cx1} ${y1 + lipIns} L ${cx1 + lipS} ${y1 + lipIns + lipSlant} L ${cx1 + lipS} ${y2 - lipIns - lipSlant} L ${cx1} ${y2 - lipIns} L ${cx1} ${y2}`
    ),
...
// src/core/templates/fefco0427.ts
function generateFefco0427(box, mat) {
  const { W, D, H } = box;
  const t = mat.thickness;
  const Wp = W + 2 * t;
  const Dp = D + 2 * t;
  ...
  const slotOff = Math.max(2, 2.5 * t);
  ...
  const panels = [
    ...
    // จึงต้องดันทางลบเพื่อให้แผ่นม้วนลอยเข้าด้านใน ไม่ใช่ทะลุออกนอกผนัง
    {
      id: "roll-left",
      parentId: "side-left",
      outline: rollOutline(xr1, xr0, 0).pts,
      hingeA: P(xr1, y3 + rollIns),
      hingeB: P(xr1, y4 - rollIns),
      foldAngle: -180,
      stage: 2,
      zOffset: -2 * layer
    },
    {
      id: "roll-right",
      parentId: "side-right",
      outline: rollOutline(xr2, xr3, width).pts,
      hingeA: P(xr2, y3 + rollIns),
      hingeB: P(xr2, y4 - rollIns),
      foldAngle: 180,
      stage: 2,
      zOffset: -2 * layer
    },
  ...
  const cut = (d) => ({ kind: "cut", d });
  const crease = (d) => ({ kind: "crease", d });
  const earCut = (hx, dir, ya, yb) => {
    const sy = Math.sign(yb - ya) || 1;
    return `M ${hx} ${ya + sy * earIns} L ${hx + dir * earW} ${ya + sy * (earIns + earSlant)} L ${hx + dir * earW} ${yb - sy * (topIns + earSlant)} L ${hx} ${yb - sy * topIns}`;
  };
  const rollFreeCut = (x1, tabX) => {
    let d = `M ${x1} ${y3 + rollIns}`;
    for (const yc of [yc1, yc2]) {
      d += ` L ${x1} ${yc - tabW / 2} L ${tabX} ${yc - tabW / 2 + 1.5} L ${tabX} ${yc + tabW / 2 - 1.5} L ${x1} ${yc + tabW / 2}`;
    }
    d += ` L ${x1} ${y4 - rollIns}`;
    return d;
  };
  const segments = [
    ...
    cut(
      `M ${cx0 + tuckIn} ${y1} L ${cx0 + tuckIn - lockW} ${y1 - lockH * 0.35} L ${cx0 + tuckIn} ${y1 - lockH} L ${cx0 + tuckIn} ${r} Q ${cx0 + tuckIn} 0 ${cx0 + tuckIn + r} 0 L ${cx1 - tuckIn - r} 0 Q ${cx1 - tuckIn} 0 ${cx1 - tuckIn} ${r} L ${cx1 - tuckIn} ${y1 - lockH} L ${cx1 - tuckIn + lockW} ${y1 - lockH * 0.35} L ${cx1 - tuckIn} ${y1}`
    ),
    cut(`M ${cx0} ${y1} L ${cx0 + tuckIn} ${y1}`),
    cut(`M ${cx1 - tuckIn} ${y1} L ${cx1} ${y1}`),
...
// src/core/templates/sleeve.ts
function generateSleeve(box, mat) {
  const { W, D, H } = box;
  const t = mat.thickness;
  const Wp = W + 2 * t;
  const Dp = D + 2 * t;
...
// src/core/templates/bottleCarrier.ts
function generateBottleCarrier(box, mat) {
  const { W, D, H } = box;
  const t = mat.thickness;
  const Wp = W + 2 * t;
  const Dp = D + 2 * t;
  ...
  const windows = (yTop, yBot) => {
    const cy = (yTop + yBot) / 2;
    return [x0 + 0.28 * Wp, x0 + 0.72 * Wp].map(
      (cx) => roundedRectPts(cx - winW / 2, cy - winH / 2, cx + winW / 2, cy + winH / 2, winR)
    );
  };
  const windowPaths = (yTop, yBot) => {
    const cy = (yTop + yBot) / 2;
    return [x0 + 0.28 * Wp, x0 + 0.72 * Wp].map(
      (cx) => roundedRectPath(cx - winW / 2, cy - winH / 2, cx + winW / 2, cy + winH / 2, winR)
    );
  };
  const panels = [
    ...
    },
    {
      id: "flap-a-right",
      parentId: "side-a",
      outline: rect(x1, y5, x1 + flapD, y6),
      hingeA: P(x1, y5),
      hingeB: P(x1, y6),
      foldAngle: 90,
      stage: 1
    ...
    },
    {
      id: "flap-b-right",
      parentId: "side-b",
      outline: rect(x1, y0, x1 + flapD, y1),
      hingeA: P(x1, y0),
      hingeB: P(x1, y1),
      foldAngle: 90,
      stage: 3
...
// src/core/templates/tray.ts
function generateTrayBox(box, mat) {
  const { W, D, H } = box;
  const t = mat.thickness;
  const Wp = W + 2 * t;
  const Dp = D + 2 * t;
  ...
  const panels = [
    ...
    },
    {
      id: "front",
      parentId: "base",
      outline: rect(cx0, by1, cx1, height),
      hingeA: P(cx0, by1),
      hingeB: P(cx1, by1),
      foldAngle: -90,
      stage: 0
    ...
    },
    {
      id: "right",
      parentId: "base",
      outline: rect(cx1, by0, width, by1),
      hingeA: P(cx1, by0),
      hingeB: P(cx1, by1),
      foldAngle: 90,
      stage: 0
  ...
  const cut = (d) => ({ kind: "cut", d });
  const crease = (d) => ({ kind: "crease", d });
  const tabCut = (xa, xb, y, dir) => `M ${xa} ${y} L ${xa + tabIn} ${y} L ${xa + tabIn + tabSlant} ${y + dir * tabW} L ${xb - tabIn - tabSlant} ${y + dir * tabW} L ${xb - tabIn} ${y} L ${xb} ${y}`;
  const segments = [
    // ขอบนอก (cut) ไล่รอบรูป
    cut(`M ${cx0} 0 L ${cx1} 0`),
    // ขอบบนผนังหลัง
    cut(`M ${cx1} 0 L ${cx1} ${by0}`),
    // ขอบขวาผนังหลัง
    ...
    // ลิ้นมุมขวา-หน้า
    cut(`M ${cx1} ${by1} L ${cx1} ${height}`),
    // ขอบขวาผนังหน้า
    cut(`M ${cx1} ${height} L ${cx0} ${height}`),
    // ขอบล่างผนังหน้า
    cut(`M ${cx0} ${by1} L ${cx0} ${height}`),
    // ขอบซ้ายผนังหน้า
    ...
    // ฐาน|ผนังขวา
    crease(`M ${tabIn} ${by0} L ${cx0 - tabIn} ${by0}`),
    // ผนังซ้าย|ลิ้นหลัง
    crease(`M ${tabIn} ${by1} L ${cx0 - tabIn} ${by1}`),
    // ผนังซ้าย|ลิ้นหน้า
    crease(`M ${cx1 + tabIn} ${by0} L ${width - tabIn} ${by0}`),
    // ผนังขวา|ลิ้นหลัง
    crease(`M ${cx1 + tabIn} ${by1} L ${width - tabIn} ${by1}`)
    // ผนังขวา|ลิ้นหน้า
  ];
  const dims = [
...
// src/core/templates/gable.ts
function generateGableBox(box, mat) {
  const { W, D, H } = box;
  const t = mat.thickness;
  const Wp = W + 2 * t;
  const Dp = D + 2 * t;
  ...
  const dHole = holeThick / 2 + Math.max(6, G * 0.16);
  const backHoleCy = dHole;
  const frontHoleCy = height - dHole;
  const tab = (xa, xb, y, dir) => [
    P(xa + tabIn, y),
    P(xa + tabIn + tabSlant, y + dir * tabW),
    P(xb - tabIn - tabSlant, y + dir * tabW),
  ...
  const panels = [
    ...
    // ผนังหน้า-หลัง — พับขึ้นทีหลัง (stage 2) ปิดทับลิ้นมุม
    {
      id: "back",
      parentId: "base",
      outline: rect(cx0, backRim, cx1, by0),
      hingeA: P(cx0, by0),
      hingeB: P(cx1, by0),
      foldAngle: 90,
      stage: 2
    },
    {
      id: "front",
      parentId: "base",
      outline: rect(cx0, by1, cx1, frontRim),
      hingeA: P(cx0, by1),
      hingeB: P(cx1, by1),
      foldAngle: -90,
      stage: 2
    ...
    // (ผนังหลังพับ +90 → จั่วหลัง +lean, ผนังหน้าพับ -90 → จั่วหน้า -lean)
    {
      id: "gable-back",
      parentId: "back",
      outline: rect(cx0, 0, cx1, backRim),
      holes: hasHandle ? [obroundPts(holeCx, backHoleCy, holeLen, holeThick)] : void 0,
      hingeA: P(cx0, backRim),
      hingeB: P(cx1, backRim),
      foldAngle: lean,
      stage: 3
    },
    {
      id: "gable-front",
      parentId: "front",
      outline: rect(cx0, frontRim, cx1, height),
      holes: hasHandle ? [obroundPts(holeCx, frontHoleCy, holeLen, holeThick)] : void 0,
      hingeA: P(cx0, frontRim),
      hingeB: P(cx1, frontRim),
      foldAngle: -lean,
      stage: 3
    }
  ];
  ...
  const segments = [
    ...
    // รอยพับ
    crease(`M ${cx0} ${backRim} L ${cx1} ${backRim}`),
    // จั่วหลัง|ผนังหลัง
    crease(`M ${cx0} ${by0} L ${cx1} ${by0}`),
    // ฐาน|ผนังหลัง
    crease(`M ${cx0} ${by1} L ${cx1} ${by1}`),
    // ฐาน|ผนังหน้า
    crease(`M ${cx0} ${frontRim} L ${cx1} ${frontRim}`),
    // ผนังหน้า|จั่วหน้า
    crease(`M ${cx0} ${by0} L ${cx0} ${by1}`),
    // ฐาน|ผนังซ้าย
    crease(`M ${cx1} ${by0} L ${cx1} ${by1}`),
...
// src/core/templates/rsc.ts
function generateRSCBox(box, mat) {
  const { W, D, H } = box;
  const t = mat.thickness;
  const Wp = W + 2 * t;
  const Dp = D + 2 * t;
...
// src/core/templates/card.ts
var GAP = 10;
function generateCard(box) {
  const { W: w, H: h } = box;
  const bx = w + GAP;
  const panels = [
  ...
  const cut = (d) => ({ kind: "cut", d });
  const rectCut = (x0) => cut(`M ${x0} 0 L ${x0 + w} 0 L ${x0 + w} ${h} L ${x0} ${h} Z`);
  const segments = [rectCut(0), rectCut(bx)];
  const width = bx + w;
  const dims = [
...
var TEMPLATES = [
  ...
  },
  {
    ...
    detail: "\u0E01\u0E32\u0E23\u0E4C\u0E14\u0E41\u0E1A\u0E19\u0E1E\u0E34\u0E21\u0E1E\u0E4C \u0E02\u0E19\u0E32\u0E14\u0E21\u0E32\u0E15\u0E23\u0E10\u0E32\u0E19 90\xD754 \u0E21\u0E21. \u2014 \u0E2D\u0E2D\u0E01\u0E41\u0E1A\u0E1A\u0E44\u0E14\u0E49\u0E17\u0E31\u0E49\u0E07\u0E14\u0E49\u0E32\u0E19\u0E2B\u0E19\u0E49\u0E32\u0E41\u0E25\u0E30\u0E14\u0E49\u0E32\u0E19\u0E2B\u0E25\u0E31\u0E07 \u0E44\u0E21\u0E48\u0E21\u0E35\u0E23\u0E2D\u0E22\u0E1E\u0E31\u0E1A",
...
var LIMITS = { W: [30, 250], D: [20, 150], H: [30, 300] };
var TEMPLATE_IDS = TEMPLATES.map((t) => t.id);
var clamp = (v, [lo, hi]) => Math.min(hi, Math.max(lo, Math.round(v * 2) / 2));
var COMMON_ITEMS = [
  { re: /แยม/, w: 65, d: 65, h: 110, label: "\u0E02\u0E27\u0E14\u0E41\u0E22\u0E21 200 \u0E21\u0E25. \u2248 \u230065\xD7110 \u0E21\u0E21.", fragile: true },
  { re: /ขวดซอส|น้ำพริก/, w: 60, d: 60, h: 160, label: "\u0E02\u0E27\u0E14\u0E0B\u0E2D\u0E2A 250 \u0E21\u0E25. \u2248 \u230060\xD7160 \u0E21\u0E21.", fragile: true },
  { re: /ขวดน้ำ(?!พริก)|เครื่องดื่ม/, w: 65, d: 65, h: 210, label: "\u0E02\u0E27\u0E14\u0E19\u0E49\u0E33 500 \u0E21\u0E25. \u2248 \u230065\xD7210 \u0E21\u0E21.", fragile: true },
  { re: /สบู่/, w: 90, d: 60, h: 28, label: "\u0E2A\u0E1A\u0E39\u0E48\u0E01\u0E49\u0E2D\u0E19 \u2248 90\xD760\xD728 \u0E21\u0E21.", fragile: false },
  { re: /แก้วมัค|แก้วกาแฟ|มัค/, w: 105, d: 85, h: 95, label: "\u0E41\u0E01\u0E49\u0E27\u0E21\u0E31\u0E04 \u2248 \u230085\xD795 \u0E21\u0E21. (\u0E23\u0E27\u0E21\u0E2B\u0E39\u0E2D\u0E35\u0E01 20)", fragile: true },
  { re: /เทียน/, w: 70, d: 70, h: 85, label: "\u0E40\u0E17\u0E35\u0E22\u0E19\u0E2B\u0E2D\u0E21\u0E43\u0E19\u0E41\u0E01\u0E49\u0E27 \u2248 \u230070\xD785 \u0E21\u0E21.", fragile: true },
  { re: /ครีม|กระปุก/, w: 55, d: 55, h: 50, label: "\u0E01\u0E23\u0E30\u0E1B\u0E38\u0E01\u0E04\u0E23\u0E35\u0E21 50 \u0E21\u0E25. \u2248 \u230055\xD750 \u0E21\u0E21.", fragile: false },
  { re: /คุกกี้|ขนม/, w: 120, d: 45, h: 180, label: "\u0E0B\u0E2D\u0E07\u0E04\u0E38\u0E01\u0E01\u0E35\u0E49 \u2248 120\xD745\xD7180 \u0E21\u0E21.", fragile: false }
];
var SYSTEM = `\u0E04\u0E38\u0E13\u0E04\u0E37\u0E2D\u0E27\u0E34\u0E28\u0E27\u0E01\u0E23\u0E1A\u0E23\u0E23\u0E08\u0E38\u0E20\u0E31\u0E13\u0E11\u0E4C \u0E2B\u0E19\u0E49\u0E32\u0E17\u0E35\u0E48\u0E04\u0E37\u0E2D\u0E41\u0E1B\u0E25\u0E07\u0E04\u0E33\u0E2D\u0E18\u0E34\u0E1A\u0E32\u0E22\u0E02\u0E2D\u0E07\u0E25\u0E39\u0E01\u0E04\u0E49\u0E32 (\u0E20\u0E32\u0E29\u0E32\u0E1E\u0E39\u0E14) \u0E40\u0E1B\u0E47\u0E19\u0E2A\u0E40\u0E1B\u0E01\u0E01\u0E25\u0E48\u0E2D\u0E07\u0E17\u0E35\u0E48\u0E04\u0E33\u0E19\u0E27\u0E13\u0E41\u0E25\u0E49\u0E27 \u0E42\u0E14\u0E22\u0E40\u0E23\u0E35\u0E22\u0E01 tool box_spec \u0E40\u0E2A\u0E21\u0E2D
...
- sleeve: W\xD7D = \u0E2B\u0E19\u0E49\u0E32\u0E15\u0E31\u0E14\u0E14\u0E49\u0E32\u0E19\u0E43\u0E19\u0E02\u0E2D\u0E07\u0E17\u0E48\u0E2D (\u0E15\u0E49\u0E2D\u0E07\u0E1E\u0E2D\u0E14\u0E35\u0E01\u0E31\u0E1A\u0E02\u0E2D\u0E07\u0E17\u0E35\u0E48\u0E2A\u0E27\u0E21 \u0E40\u0E0A\u0E48\u0E19 \u0E41\u0E01\u0E49\u0E27 \u230090 \u2192 W\u224892, D\u224892), H = \u0E04\u0E27\u0E32\u0E21\u0E2A\u0E39\u0E07\u0E02\u0E2D\u0E07\u0E1B\u0E25\u0E2D\u0E01 \u2014 \u0E23\u0E30\u0E1A\u0E1A\u0E04\u0E33\u0E19\u0E27\u0E13\u0E04\u0E27\u0E32\u0E21\u0E22\u0E32\u0E27\u0E41\u0E1C\u0E48\u0E19\u0E1E\u0E31\u0E19\u0E23\u0E2D\u0E1A (\u0E1B\u0E35\u0E01\u0E01\u0E32\u0E27 + 2(W+D)) \u0E43\u0E2B\u0E49\u0E40\u0E2D\u0E07\u0E2D\u0E22\u0E39\u0E48\u0E41\u0E25\u0E49\u0E27 \u0E2B\u0E49\u0E32\u0E21\u0E40\u0E2D\u0E32\u0E40\u0E2A\u0E49\u0E19\u0E23\u0E2D\u0E1A\u0E27\u0E07\u0E02\u0E2D\u0E07\u0E21\u0E32\u0E43\u0E2A\u0E48\u0E43\u0E19 W \u0E40\u0E14\u0E47\u0E14\u0E02\u0E32\u0E14
- sleeve \u0E40\u0E1B\u0E47\u0E19\u0E17\u0E48\u0E2D\u0E17\u0E23\u0E07\u0E15\u0E23\u0E07 \u2014 \u0E02\u0E2D\u0E07\u0E17\u0E23\u0E07\u0E40\u0E23\u0E35\u0E22\u0E27 (\u0E41\u0E01\u0E49\u0E27\u0E01\u0E32\u0E41\u0E1F) \u0E08\u0E30\u0E2B\u0E25\u0E27\u0E21\u0E14\u0E49\u0E32\u0E19\u0E41\u0E04\u0E1A \u0E43\u0E2B\u0E49\u0E1B\u0E23\u0E30\u0E01\u0E32\u0E28\u0E40\u0E1B\u0E47\u0E19\u0E02\u0E49\u0E2D\u0E08\u0E33\u0E01\u0E31\u0E14\u0E43\u0E19 assumptions
- bottle-carrier: W\xD7D = footprint \u0E14\u0E49\u0E32\u0E19\u0E43\u0E19 (\u0E40\u0E0A\u0E48\u0E19 \u0E02\u0E27\u0E14 \u230066 \u0E27\u0E32\u0E07 2\xD72 \u2192 W,D \u2248 150), H = \u0E04\u0E27\u0E32\u0E21\u0E2A\u0E39\u0E07\u0E02\u0E27\u0E14 \u2014 \u0E23\u0E30\u0E1A\u0E1A\u0E2A\u0E23\u0E49\u0E32\u0E07\u0E43\u0E2B\u0E49\u0E40\u0E2D\u0E07\u0E2D\u0E31\u0E15\u0E42\u0E19\u0E21\u0E31\u0E15\u0E34: \u0E41\u0E1C\u0E48\u0E19\u0E2B\u0E39\u0E2B\u0E34\u0E49\u0E27\u0E01\u0E25\u0E32\u0E07\u0E2A\u0E39\u0E07\u0E01\u0E27\u0E48\u0E32\u0E02\u0E27\u0E14 ~55 \u0E21\u0E21. \u0E1E\u0E23\u0E49\u0E2D\u0E21\u0E23\u0E39\u0E21\u0E37\u0E2D\u0E01\u0E25\u0E21, \u0E1C\u0E19\u0E31\u0E07\u0E02\u0E49\u0E32\u0E07\u0E2A\u0E39\u0E07 ~42% \u0E02\u0E2D\u0E07 H, \u0E2B\u0E19\u0E49\u0E32\u0E15\u0E48\u0E32\u0E07\u0E42\u0E0A\u0E27\u0E4C\u0E2A\u0E34\u0E19\u0E04\u0E49\u0E32\u0E2A\u0E2D\u0E07\u0E0A\u0E48\u0E2D\u0E07\u0E15\u0E48\u0E2D\u0E1C\u0E19\u0E31\u0E07 (handle flag \u0E44\u0E21\u0E48\u0E40\u0E01\u0E35\u0E48\u0E22\u0E27\u0E01\u0E31\u0E1A template \u0E19\u0E35\u0E49 \u2014 \u0E2B\u0E39\u0E2B\u0E34\u0E49\u0E27\u0E21\u0E35\u0E43\u0E19\u0E15\u0E31\u0E27)
- card (\u0E19\u0E32\u0E21\u0E1A\u0E31\u0E15\u0E23): \u0E01\u0E32\u0E23\u0E4C\u0E14\u0E41\u0E1A\u0E19\u0E1E\u0E34\u0E21\u0E1E\u0E4C\u0E2A\u0E2D\u0E07\u0E2B\u0E19\u0E49\u0E32 (\u0E2B\u0E19\u0E49\u0E32+\u0E2B\u0E25\u0E31\u0E07 \u0E27\u0E32\u0E07\u0E40\u0E23\u0E35\u0E22\u0E07\u0E01\u0E31\u0E19\u0E1A\u0E19\u0E41\u0E1C\u0E48\u0E19\u0E2D\u0E2D\u0E01\u0E41\u0E1A\u0E1A \u0E15\u0E01\u0E41\u0E15\u0E48\u0E07\u0E44\u0E14\u0E49\u0E17\u0E31\u0E49\u0E07\u0E2A\u0E2D\u0E07\u0E14\u0E49\u0E32\u0E19) \u0E44\u0E21\u0E48\u0E21\u0E35\u0E23\u0E2D\u0E22\u0E1E\u0E31\u0E1A \u2014 W = \u0E01\u0E27\u0E49\u0E32\u0E07, H = \u0E2A\u0E39\u0E07, \u0E04\u0E48\u0E32 D \u0E44\u0E21\u0E48\u0E43\u0E0A\u0E49 (\u0E43\u0E2A\u0E48\u0E04\u0E48\u0E32\u0E43\u0E14\u0E01\u0E47\u0E44\u0E14\u0E49\u0E43\u0E19\u0E25\u0E34\u0E21\u0E34\u0E15), handle \u0E15\u0E49\u0E2D\u0E07\u0E40\u0E1B\u0E47\u0E19 false; \u0E02\u0E19\u0E32\u0E14\u0E21\u0E32\u0E15\u0E23\u0E10\u0E32\u0E19 90\xD754 \u0E21\u0E21. \u0E40\u0E25\u0E37\u0E2D\u0E01\u0E40\u0E21\u0E37\u0E48\u0E2D\u0E25\u0E39\u0E01\u0E04\u0E49\u0E32\u0E02\u0E2D "\u0E19\u0E32\u0E21\u0E1A\u0E31\u0E15\u0E23 / business card" \u0E40\u0E17\u0E48\u0E32\u0E19\u0E31\u0E49\u0E19 (\u0E40\u0E1B\u0E47\u0E19\u0E07\u0E32\u0E19\u0E1E\u0E34\u0E21\u0E1E\u0E4C\u0E01\u0E32\u0E23\u0E4C\u0E14 \u0E44\u0E21\u0E48\u0E43\u0E0A\u0E48\u0E1A\u0E23\u0E23\u0E08\u0E38\u0E20\u0E31\u0E13\u0E11\u0E4C)

## \u0E27\u0E31\u0E2A\u0E14\u0E38\u0E17\u0E35\u0E48\u0E21\u0E35\u0E43\u0E19\u0E23\u0E30\u0E1A\u0E1A (materialId)
${MATERIALS.map(
  (m) => `- ${m.id}: ${m.nameTh} (${m.detail}) \u0E2B\u0E19\u0E32 ${m.thickness} \u0E21\u0E21. ${m.foldable ? "\u0E1E\u0E31\u0E1A\u0E44\u0E14\u0E49" : "\u0E1E\u0E31\u0E1A\u0E44\u0E21\u0E48\u0E44\u0E14\u0E49 \u2014 " + m.process}`
).join("\n")}
...
2. \u0E08\u0E31\u0E14 layout \u0E41\u0E16\u0E27 \xD7 \u0E04\u0E2D\u0E25\u0E31\u0E21\u0E19\u0E4C: W = \u0E04\u0E2D\u0E25\u0E31\u0E21\u0E19\u0E4C\xD7\u0E01\u0E27\u0E49\u0E32\u0E07\u0E0A\u0E34\u0E49\u0E19 + \u0E0A\u0E48\u0E2D\u0E07\u0E44\u0E1F, D = \u0E41\u0E16\u0E27\xD7\u0E25\u0E36\u0E01\u0E0A\u0E34\u0E49\u0E19 + \u0E0A\u0E48\u0E2D\u0E07\u0E44\u0E1F
...
1. \u0E04\u0E33\u0E19\u0E27\u0E13\u0E1E\u0E37\u0E49\u0E19\u0E17\u0E35\u0E48\u0E17\u0E35\u0E48\u0E02\u0E2D\u0E07\u0E15\u0E49\u0E2D\u0E07\u0E43\u0E0A\u0E49\u0E08\u0E23\u0E34\u0E07 (\u0E23\u0E27\u0E21\u0E0A\u0E48\u0E2D\u0E07\u0E44\u0E1F/\u0E01\u0E31\u0E19\u0E01\u0E23\u0E30\u0E41\u0E17\u0E01) \u0E40\u0E17\u0E35\u0E22\u0E1A\u0E01\u0E31\u0E1A W\xD7D\xD7H \u0E17\u0E35\u0E48\u0E08\u0E30\u0E15\u0E2D\u0E1A \u2014 \u0E16\u0E49\u0E32\u0E43\u0E2A\u0E48\u0E44\u0E21\u0E48\u0E44\u0E14\u0E49\u0E08\u0E23\u0E34\u0E07 \u0E2B\u0E49\u0E32\u0E21\u0E40\u0E02\u0E35\u0E22\u0E19 assumptions/layoutNote/reasoning \u0E40\u0E2B\u0E21\u0E37\u0E2D\u0E19\u0E43\u0E2A\u0E48\u0E44\u0E14\u0E49
...
- layoutNote \u0E2A\u0E31\u0E49\u0E19\u0E46 \u0E40\u0E0A\u0E48\u0E19 "\u0E27\u0E32\u0E07\u0E40\u0E23\u0E35\u0E22\u0E07 3 \xD7 1" (\u0E16\u0E49\u0E32\u0E44\u0E21\u0E48\u0E40\u0E01\u0E35\u0E48\u0E22\u0E27\u0E01\u0E31\u0E1A\u0E01\u0E32\u0E23\u0E27\u0E32\u0E07\u0E02\u0E2D\u0E07 \u0E43\u0E2A\u0E48 "-")
...
var TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      layoutNote: { type: "string", description: '\u0E01\u0E32\u0E23\u0E08\u0E31\u0E14\u0E27\u0E32\u0E07\u0E02\u0E2D\u0E07\u0E14\u0E49\u0E32\u0E19\u0E43\u0E19 \u0E40\u0E0A\u0E48\u0E19 "\u0E27\u0E32\u0E07\u0E40\u0E23\u0E35\u0E22\u0E07 3 \xD7 1"' },
...
function sanitize(raw, mock) {
  const materialId = MATERIALS.some((m) => m.id === raw.materialId) ? raw.materialId : "carton-300";
  const template = TEMPLATE_IDS.includes(raw.template) ? raw.template : "tuck-end";
  return {
    template,
    materialId,
    ...
    assumptions: Array.isArray(raw.assumptions) ? raw.assumptions.filter((a) => typeof a === "string").slice(0, 8) : [],
...
function parseCurrent(v) {
  ...
  const D = Number(o.D);
  const H = Number(o.H);
  if (!Number.isFinite(W) || !Number.isFinite(D) || !Number.isFinite(H)) return void 0;
  const materialId = MATERIALS.some((m) => m.id === o.materialId) ? o.materialId : void 0;
  const template = TEMPLATE_IDS.includes(o.template) ? o.template : void 0;
  if (!materialId || !template) return void 0;
  return {
    template,
    materialId,
    W: clamp(W, LIMITS.W),
...
function parseImage(v) {
  if (typeof v !== "object" || v === null) return void 0;
  const o = v;
  if (typeof o.data !== "string" || o.data.length < 100 || o.data.length > 5e6) return void 0;
  const data = o.data.replace(/\s/g, "");
  if (!/^[A-Za-z0-9+/]+=*$/.test(data)) return void 0;
  const mediaType = IMAGE_TYPES.has(o.mediaType) ? o.mediaType : "image/jpeg";
  return { data, mediaType };
}
function mockSpec(prompt, current, image) {
  const assumptions = [];
  ...
  if (/นามบัตร|business ?card/i.test(prompt)) {
    assumptions.push("\u0E23\u0E39\u0E1B\u0E41\u0E1A\u0E1A: \u0E19\u0E32\u0E21\u0E1A\u0E31\u0E15\u0E23 (\u0E01\u0E32\u0E23\u0E4C\u0E14\u0E41\u0E1A\u0E19) \u0E02\u0E19\u0E32\u0E14\u0E21\u0E32\u0E15\u0E23\u0E10\u0E32\u0E19 90\xD754 \u0E21\u0E21.");
    return sanitize(
      {
        ...
        assumptions,
        layoutNote: "-",
        reasoning: "\u0E19\u0E32\u0E21\u0E1A\u0E31\u0E15\u0E23\u0E21\u0E32\u0E15\u0E23\u0E10\u0E32\u0E19 90\xD754 \u0E21\u0E21. (\u0E42\u0E2B\u0E21\u0E14\u0E08\u0E33\u0E25\u0E2D\u0E07)"
      },
      true
    );
  }
  ...
  if (/ถุง|ซอง|doypack|ซิปล็อก|ซิป|pouch|stand.?up/i.test(prompt)) {
    materialId = /คราฟท์|อีโค่|รักษ์โลก/.test(prompt) ? "pouch-kraft" : /ใส(?![่-๋])|โชว์|มองเห็น/.test(prompt) ? "pouch-clear" : "pouch-foil";
    handle = false;
  ...
  if (!item) {
    ...
    assumptions.push("\u0E44\u0E21\u0E48\u0E17\u0E23\u0E32\u0E1A\u0E02\u0E2D\u0E07\u0E14\u0E49\u0E32\u0E19\u0E43\u0E19 \u2014 \u0E43\u0E0A\u0E49\u0E02\u0E19\u0E32\u0E14\u0E01\u0E25\u0E48\u0E2D\u0E07\u0E21\u0E32\u0E15\u0E23\u0E10\u0E32\u0E19 80\xD750\xD7120 \u0E21\u0E21.");
    return sanitize(
  ...
  return sanitize(
    {
      ...
      H,
      assumptions,
      layoutNote: `\u0E27\u0E32\u0E07\u0E40\u0E23\u0E35\u0E22\u0E07 ${cols} \xD7 ${rows}`,
...
function buildUserContent(prompt, current) {
  return current ? `\u0E2A\u0E40\u0E1B\u0E01\u0E1B\u0E31\u0E08\u0E08\u0E38\u0E1A\u0E31\u0E19: template=${current.template}, \u0E27\u0E31\u0E2A\u0E14\u0E38=${current.materialId}, \u0E14\u0E49\u0E32\u0E19\u0E43\u0E19 W\xD7D\xD7H = ${current.W}\xD7${current.D}\xD7${current.H} \u0E21\u0E21., \u0E23\u0E39\u0E2B\u0E34\u0E49\u0E27=${current.handle ? "\u0E21\u0E35" : "\u0E44\u0E21\u0E48\u0E21\u0E35"}
...
async function askClaude(apiKey, model, prompt, current, image) {
  ...
  const block = msg.content.find(
    (b) => b.type === "tool_use" && b.name === "box_spec"
  );
  if (!block) throw new Error("\u0E44\u0E21\u0E48\u0E1E\u0E1A\u0E1C\u0E25\u0E25\u0E31\u0E1E\u0E18\u0E4C\u0E08\u0E32\u0E01\u0E42\u0E21\u0E40\u0E14\u0E25");
  return sanitize(block.input, false);
}
var JSON_INSTRUCTION = `
...
async function handler(req, res) {
  ...
  if (typeof body.prompt !== "string" || !body.prompt.trim() || body.prompt.length > 2e3) {
