...
// server/config.ts
var APP_ENVIRONMENTS = ["development", "test", "staging", "production"];
...
function parseAppEnvironment(value) {
  ...
  if (APP_ENVIRONMENTS.includes(appEnv)) return appEnv;
...
function toApiFailure(error, requestId) {
  return {
    error: {
      code: error.code,
      message: error.message,
...
// server/adapters/supabase/sessionService.ts
function normalizedDisplayName(user) {
  const metadataName = user.user_metadata.full_name ?? user.user_metadata.name;
  const candidate = typeof metadataName === "string" && metadataName.trim() ? metadataName.trim() : user.email?.split("@")[0]?.trim() || "\u0E1C\u0E39\u0E49\u0E43\u0E0A\u0E49 PackIt";
...
var SupabaseSessionService = class {
  constructor(client, issuer) {
    this.client = client;
    this.issuer = issuer;
  }
  async bootstrap(accessToken, _requestId) {
...
function createSupabaseSessionService(config) {
...
function createApiRouter(config, dependencies = {}) {
  const sessionService = dependencies.sessionService === void 0 ? config.supabase && createSupabaseSessionService(config.supabase) : dependencies.sessionService;
  return async (req, res) => {
    ...
    if (path === "/api/v1/health") {
      if (req.method !== "GET") {
        ...
        const error2 = new HttpError(405, "METHOD_NOT_ALLOWED", "Method \u0E19\u0E35\u0E49\u0E43\u0E0A\u0E49\u0E01\u0E31\u0E1A endpoint \u0E44\u0E21\u0E48\u0E44\u0E14\u0E49");
        sendJson(res, error2.status, toApiFailure(error2, requestId), requestId);
        return;
      }
      const body = { data: { status: "ok" }, requestId };
    ...
    if (path === "/api/v1/session/bootstrap") {
      ...
      try {
        ...
        const body = { data, requestId };
        sendJson(res, 200, body, requestId);
      } catch (error2) {
        const httpError = error2 instanceof HttpError ? error2 : new HttpError(503, "DEPENDENCY_UNAVAILABLE", "\u0E15\u0E23\u0E27\u0E08\u0E2A\u0E2D\u0E1A session \u0E44\u0E21\u0E48\u0E2A\u0E33\u0E40\u0E23\u0E47\u0E08");
